...
export default function EditPDF() {
  ...
  const dragTempPosRef = useRef<{ x: number; y: number } | null>(null);
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isDraggingImage && selectedAnnotation) {
      ...
      dragTempPosRef.current = { x: nx, y: ny };
        ...
        annotations
          ...
          .forEach((a) => {
            if (a.id === selectedAnnotation) {
              renderAnnotation(ctx, { ...a, x: nx, y: ny }, true);
            } else {
              renderAnnotation(ctx, a, false);
            }
          });
      }
      return;
  ...
  const handleCanvasMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (isDraggingImage) {
      ...
      if (dragTempPosRef.current && selectedAnnotation) {
        updateAnnotation(selectedAnnotation, { 
          x: dragTempPosRef.current.x, 
          y: dragTempPosRef.current.y 
        }, true);
        dragTempPosRef.current = null;
      } else {
        const nh = history.slice(0, historyIndex + 1);
        nh.push([...annotations]);
        setHistory(nh);
